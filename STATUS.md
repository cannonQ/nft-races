...
- **Single-TX batch training** — Train up to 20 creatures in one TX. Grid matrix UI with per-creature activity radio buttons, default activity pills, auto-apply boosts/recovery packs, expandable detail rows, inline MiniStatBars per creature (colored vertical bars with per-bar hover), and activity column stat dot indicators with hover tooltips showing gain details. Compact cooldown indicator per creature row (green pulsing "Ready" dot or clock icon with countdown). Creatures on cooldown are greyed out and unselectable. Cooldown data uses a 7-hour query window (separate from the daily-action-count query) to correctly span UTC midnight boundaries. Same N-output TX structure as batch race entry (R4=train, R5=tokenId, R6=activity per output). Same-collection enforced. Nautilus ERG + CYPX verified on mainnet. ErgoPay verified on live preview. Endpoint: `POST /api/v2/train-batch`.
